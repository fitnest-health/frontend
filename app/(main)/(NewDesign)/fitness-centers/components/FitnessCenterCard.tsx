...
const FitnessCenterCard = ({
  ...
  const membershipLabelMap: Record<FitnessCenterCardProps["membership"], string> =
    {
      bronze: "Bronze",
      silver: "Silver",
      gold: "Gold",
  ...
  const membershipVariantMap: Record<
    FitnessCenterCardProps["membership"],
  ...
  return (
    <article className="flex h-full flex-col gap-6 rounded-[32px] border border-[#111729] bg-[#111729] p-7">
      ...
      <div className="space-y-4">
        <div className="flex items-start justify-between gap-3">
          <h3 className="text-xl font-semibold leading-[30px] text-white">
            {name}
          </h3>
          <Badge
        ...
        <div className="flex items-end justify-between gap-4">
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2 text-sm text-[#C1C1CC]">
              ...
              <span>İş saatları: {workHours}</span>
            </div>
          </div>
          ...
          >
            <Link href={href} aria-label={`${name} detalları`}>
